...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.red)}%n$prompt_symbol%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]%F{011}${vcs_info_msg_0_}\n%F{green}└─%B%(#.%F{red}#.%F{red}$)%b%F{reset} '
...
# Docker functions
for file in ~/.{bash_prompt,bash_aliases,functions,path,extra,exports,dockerfunc,kubefunc}; do
